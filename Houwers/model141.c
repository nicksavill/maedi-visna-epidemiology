...
double v(int, int, int, int, parameters_t*);
...
/*
A beta:1
B expected infected: 2
C expected infectious: 5
D lambda: 6
E dist of infection time for t0=0,12,24: 7,8,9
F dist of sero for t0=0,12,24: 10,11,12
*/
...
void Variables(struct variables *V) 
{
  ...
  Var(V,  0, "month");
  Var(V,  1, "beta");
  Var(V,  2, "expected number infected");
  Var(V,  3, "cumulative seroconversions");
  Var(V,  4, "seroconversion rate");
  Var(V,  5, "expected number infectious");
  Var(V,  6, "force of infection");
  Var(V,  7, "dist infections t0=0");
  Var(V,  8, "dist infections t0=12");
  Var(V,  9, "dist infections t0=24");
  Var(V, 10, "dist sero t0=0");
  Var(V, 11, "dist sero t0=12");
  Var(V, 12, "dist sero t0=24");
...
int Model(const uint trt, const uint ind, const uint nmonths, double *p, double **V, TimePoints *Data) 
{
  ...
  if (Data->mode == OUTPUT) {
    ...
    for (m = 0; m < nmonths-1; m++)
      V[m][2] = num_infected[m];
    V[m][2] = V[m-1][2];

    // expected number of infectious sheep in month m
    for (m = 0; m < nmonths-1; m++)
      V[m][5] = num_infectious[m];
    V[m][5] = V[m-1][5];

    // force of infection in month m
    for (m = 0; m < nmonths-1; m++)
      V[m][6] = pars.lambda[m];

    // dist of infection t0=0
    for (m = 0; m < nmonths-1; m++)
      V[m][7] = pars.lambda[m]*u(0, m, pars.Lambda);
    // dist of infection t0=0
    for (m = 0; m < 12; m++)
      V[m][8] = -1;
    for (m = 12; m < nmonths-1; m++)
      V[m][8] = pars.lambda[m]*u(12, m, pars.Lambda);
    // dist of infection t0=0
    for (m = 0; m < 24; m++)
      V[m][9] = -1;
    for (m = 24; m < nmonths-1; m++)
      V[m][9] = pars.lambda[m]*u(24, m, pars.Lambda);

    // monthly seroconversion rates
    // remove poor condition sero for output
    pars.b[22] = S_b1;
    // cumulative seroconversion rate
    for (m = 1; m < nmonths; m++)
      pars.B[m] = pars.B[m-1] + pars.b[m-1];
    ...
    int m0 = 0;
    for (m = m0; m < nmonths-1; m++)
      for (n = m0; n < m; n++)
        V[m][10] += u(m0, n, pars.Lambda)*(v(n, m, n+1, m, &pars) - v(n, m, n, m, &pars));
    m0 = 12;
    for (m = 0; m < m0; m++)
      V[m][11] = -1;
    for (m = m0; m < nmonths-1; m++)
      for (n = m0; n < m; n++)
        V[m][11] += u(m0, n, pars.Lambda)*(v(n, m, n+1, m, &pars) - v(n, m, n, m, &pars));
    m0 = 24;
    for (m = 0; m < m0; m++)
      V[m][12] = -1;
    for (m = m0; m < nmonths-1; m++)
      for (n = m0; n < m; n++)
        V[m][12] += u(m0, n, pars.Lambda)*(v(n, m, n+1, m, &pars) - v(n, m, n, m, &pars));
  }
...
double v(int n, int m, int t, int s, parameters_t *p)
{
  double a = p->a;
  double l = p->lambda[n];
  double b = p->b[n];
  ...
  z = r*Z;
  if (z < 0)
    x1 =  a*log(Z) + ln_inc_gamma_star(a, z);
  else
    x1 = -a*log(r) + log(P_inc_gamma(a, z));

  x = -p->b[m]*l/b*exp(-l*(t-n) - l*Z/b + x1);
  if (!isfinite(x)) {
    printf("%f %f %f %e %f\n", l, b, r, Z, x1);
    printf("%d %d %d %d\n", n, m, t, s);
    exit(0);
