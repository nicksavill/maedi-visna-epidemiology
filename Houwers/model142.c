...
#define minmax(A, B, C) (min(max((A), (B)), (C)-1))
...
int Model(const uint trt, const uint ind, const uint nmonths, double *p, double **V, TimePoints *Data) 
{
  ...
  double *Q = doublevector(nmonths);
  double *F = doublevector(nmonths);
  double *P = doublevector(nmonths);
  ...
  if (L < 0) {
    for (m = 0; m < nmonths; m++)
      Q[m] = 1.-P_inc_gamma(S_a, m);
    for (m = 0; m < nmonths; m++)
      F[m] = (Q[m]-Q[m+1])/pars.b[m];
    for (m = 0; m < nmonths; m++) {
      for (n = m; n < nmonths; n++)
        P[m] += F[n];
      P[m] *= pars.b[m];
  ...
  // calculate expected number of infectious ewes
  // the two donor sheep are infectious at start of experiment
  for (j = 0; j < NUMBER_DONORS; j++)
    for (m = 0; m < sheep_last_month[j]; m++) {
      num_infected[m]++;
      num_infectious[m]++;
    }
  // ewe 44 - infected by dam
  for (m = minmax(sheep_intro_month[44], sheep_pos_month[44]+L, nmonths); m < sheep_last_month[44]; m++) {
    num_infected[m]++;
    num_infectious[m]++;
  }
  ...
  for (j = NUMBER_DONORS; j < NUMBER_SHEEP; j++) {
    m = sheep_pos_month[j];
    if (use_sheep[j] && m != -1) {
      for (n = max(0, m+L); n <= sheep_last_month[j]; n++)
        if (n >= m)
          num_infectious[n]++;
        else
          num_infectious[n] += P[m-n];
    }
  }
