...
void Parameters(int trt, int ind, double *p, struct parameterset *Q) 
{
  ...
  // Par(Q,  5, Real, "L_{mu}", Uniform, 0., 60.);
  // Par(Q,  6, Real, "L_{sigma}", Gamma, 2.0, 15.);
  Par(Q,  7, Int,  "tau", Uniform, 1, 4);
  Par(Q,  8, Real, "gamma", Exponential, 1.0);
  // p[8] = 0.01;
...
int Model(const uint trt, const uint ind, const uint nmonths, double *p, double **V, TimePoints *Data) 
{
  ...
  for (m = 0; m < nmonths; m++) {
    L_cdf[m] = gamma*m;
    // L_cdf[m]s = gsl_cdf_gamma_P((double)m, L_a, 1./L_b);
